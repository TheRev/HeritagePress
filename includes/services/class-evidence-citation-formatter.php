...
class Evidence_Citation_Formatter {
    ...
    public function format($source, $citation = null) {
        ...
        $method_name = 'format_' . strtolower($source->type);
    ...
    private function format_will($source, $citation) {
        ...
        // Specific location within probate records
        if ($citation) {
            $details = [];
    ...
    private function format_newspaper($source, $citation) {
        ...
        // Article title (if specific article)
    ...
    private function format_land($source, $citation) {
        ...
        // Grantor to Grantee, type of document
        if ($source->title) {
            $formatted .= $source->title;
        } else {
    ...
    private function format_military($source, $citation) {
        ...
        // Service member and record type
        if ($source->title) {
            $formatted .= $source->title;
        } else {
    ...
    /**
     * Format source list entry (shorter form for bibliography)
     */
    public function format_source_list_entry($source) {
        $formatted = '';
        
        // Author/Repository (primary sort element)
        if ($source->author) {
            $formatted .= $source->author . '. ';
        ...
        // Collection (if different from title)
        if ($source->collection && $source->collection !== $source->title) {
            $formatted .= ' ' . $source->collection;
        }
        
        // Publication/Creation info
        if ($source->place || $source->publisher || $source->date_created) {
            $formatted .= ' (';
            $parts = [];
            if ($source->place) $parts[] = $source->place;
            if ($source->publisher) $parts[] = $source->publisher;
            if ($source->date_created) $parts[] = $source->date_created;
    ...
    /**
     * Format subsequent reference (short form)
     */
    public function format_subsequent_reference($source, $citation = null) {
        $formatted = '';
        ...
        if ($source->title) {
            ...
            if (count($words) > 4) {
                $formatted .= implode(' ', array_slice($words, 0, 4)) . '...';
            } else {
                $formatted .= $source->title;
            }
        }
