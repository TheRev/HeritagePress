...
/**
 * TNG-Compatible GEDCOM Import Service
 * Matches exact TNG database schema for perfect compatibility
 */
class GedcomServiceTNG
{
    private $wpdb;
    ...
    /**
     * Import GEDCOM file with exact TNG database mapping
     */
    ...
    /**
     * Import individual person record - TNG people table format
     */
    private function import_individual($record)
    {
        ...
        foreach ($record['data'] as $line) {
            if (preg_match('/^1 NAME (.+)/', $line, $matches)) {
                ...
                $data['metaphone'] = metaphone($name['surname']);
                ...
                $data['living'] = 0;
            } elseif (preg_match('/^1 BURI/', $line)) {
                $this->extract_event_data($record['data'], $line, 'BURI', $data);
    ...
    /**
     * Import family record - TNG families table format
     */
    ...
    /**
     * Import source record - TNG sources table format
     */
    private function import_source($record)
    {
        ...
        foreach ($record['data'] as $line) {
            if (preg_match('/^1 TITL (.+)/', $line, $matches)) {
                ...
                $data['shorttitle'] = substr(trim($matches[1]), 0, 50);
                ...
                $data['repoID'] = $matches[1];
            } elseif (preg_match('/^1 NOTE (.+)/', $line, $matches)) {
    ...
    /**
     * Import repository record - TNG repositories table format
     */
    ...
    /**
     * Import note record - TNG xnotes table format
     */
    ...
    /**
     * Import media record - TNG media table format
     */
    private function import_media($record)
    {
        ...
        foreach ($record['data'] as $line) {
            if (preg_match('/^1 FILE (.+)/', $line, $matches)) {
                $data['path'] = trim($matches[1]);
            } elseif (preg_match('/^1 TITL (.+)/', $line, $matches)) {
                $data['description'] = trim($matches[1]);
            } elseif (preg_match('/^2 FORM (.+)/', $line, $matches)) {
                $form = strtolower(trim($matches[1]));
                $data['form'] = $form;

                // Set media type based on form
                if (in_array($form, ['jpg', 'jpeg', 'png', 'gif', 'bmp'])) {
                    $data['mediatypeID'] = 'photos';
                } elseif (in_array($form, ['pdf', 'doc', 'txt'])) {
                    $data['mediatypeID'] = 'documents';
                } else {
                    $data['mediatypeID'] = 'photos';
                }
            }
    ...
    /**
     * Extract event data (birth/death/burial) from GEDCOM
     */
            ...
            if (preg_match('/^2 DATE (.+)/', $line, $matches)) {
                ...
                switch ($event_type) {
                    case 'BIRT':
                        ...
                        $data['birthdatetr'] = $this->parse_date($date_str);
                        break;
                    case 'DEAT':
                        $data['deathdate'] = $date_str;
                        $data['deathdatetr'] = $this->parse_date($date_str);
                        break;
                    case 'BURI':
                        $data['burialdate'] = $date_str;
                        $data['burialdatetr'] = $this->parse_date($date_str);
                        break;
                }
                ...
                switch ($event_type) {
                    ...
                    case 'DEAT':
                        ...
                        break;
                    case 'BURI':
                        $data['burialplace'] = trim($matches[1]);
                        break;
                }
            }
        }
    ...
    private function extract_marriage_data($lines, $event_line, &$data)
    {
        ...
        for ($i = $event_index + 1; $i < count($lines); $i++) {
            ...
            if (preg_match('/^2 DATE (.+)/', $line, $matches)) {
                $data['marrdate'] = trim($matches[1]);
                $data['marrdatetr'] = $this->parse_date($matches[1]);
            } elseif (preg_match('/^2 PLAC (.+)/', $line, $matches)) {
                $data['marrplace'] = trim($matches[1]);
            }
        }
    }
    ...
    private function extract_divorce_data($lines, $event_line, &$data)
    {
        ...
        for ($i = $event_index + 1; $i < count($lines); $i++) {
            ...
            if (preg_match('/^2 DATE (.+)/', $line, $matches)) {
                $data['divdate'] = trim($matches[1]);
                $data['divdatetr'] = $this->parse_date($matches[1]);
            } elseif (preg_match('/^2 PLAC (.+)/', $line, $matches)) {
                $data['divplace'] = trim($matches[1]);
            }
        }
    }
