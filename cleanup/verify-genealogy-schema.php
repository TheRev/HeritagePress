<?php
/**
 * Genealogy Schema Verification - Structure Validation
 * Verifies that all tables match genealogy software specifications exactly
 */

require_once('../../../../../../wp-config.php');

echo "<h1>🔍 Genealogy Schema Structure Verification</h1>\n";

global $wpdb;

// Define all 36 genealogy tables with their expected structure
$genealogy_tables = [
    // Core Tables (22)
...
if ($found_count >= $total_expected) {
    ...
    echo "<h3>🎉 Complete Genealogy Schema Verified!</h3>\n";
    echo "<p>All required tables are present. Your HeritagePress installation has comprehensive genealogy software compatibility.</p>\n";
    echo "</div>\n";
} else {
    echo "<div style='background: #fff3cd; padding: 15px; border-left: 4px solid #ffc107; margin: 20px 0;'>\n";
    echo "<h3>⚠️ Incomplete Schema</h3>\n";
    echo "<p>Missing " . ($total_expected - $found_count) . " tables for complete genealogy software compatibility.</p>\n";
    echo "</div>\n";
}
...
$core_tables = array_slice($genealogy_tables, 0, 22);
...
$missing_tables = [];
foreach ($genealogy_tables as $table => $fields) {
    $full_table = $wpdb->prefix . $table;
...
if (empty($missing_tables)) {
    ...
    echo "<ol>\n";
    echo "<li>Run the <code>install-complete-genealogy-schema.php</code> script</li>\n";
...
echo "<h3>Genealogy Software Compatibility Score: {$compatibility_percentage}%</h3>\n";

if ($compatibility_percentage >= 95) {
    echo "<p style='color: green; font-size: 18px;'>🏆 <strong>Excellent!</strong> Near-complete genealogy software compatibility.</p>\n";
} elseif ($compatibility_percentage >= 80) {
    echo "<p style='color: orange; font-size: 18px;'>⭐ <strong>Good!</strong> Most genealogy features available.</p>\n";
} elseif ($compatibility_percentage >= 60) {
    echo "<p style='color: orange; font-size: 18px;'>⚠️ <strong>Partial.</strong> Basic genealogy features available.</p>\n";
} else {
    echo "<p style='color: red; font-size: 18px;'>❌ <strong>Limited.</strong> Incomplete genealogy software compatibility.</p>\n";
}
...
if ($found_count >= $total_expected) {
    ...
    echo "<li>✅ All genealogy data migration paths available</li>\n";
} else {
    echo "<li>⚠️ Some GEDCOM features may not import correctly</li>\n";
    echo "<li>⚠️ Genealogy software migration may require additional setup</li>\n";
}
echo "</ul>\n";
echo "</div>\n";
...
echo "<a href='install-complete-genealogy-schema.php'>Install Complete Schema →</a></p>\n";
?>